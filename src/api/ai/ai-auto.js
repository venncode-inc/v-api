  ...
  async function getAnswer(q) {
    ...
    try {
      ...
      if (fontsRes.status === 200) {
        return fontsRes.data.result || fontsRes.data.message;
      }
    } catch (e) {
    ...
    try {
      ...
      if (googleRes.status === 200) {
        return googleRes.data.result || googleRes.data.message;
      }
    } catch (e) {
      if (e.response?.status !== 500) throw e;
    }
    ...
    try {
      ...
      if (geminiRes.status === 200) {
        return geminiRes.data.result || geminiRes.data.message;
      }
    } catch (e) {
  ...
  app.get('/ai/auto', async (req, res) => {
    try {
      ...
      res.status(200).json({
        status: true,
        creator: "Hazel",
        result: result || "Tidak ada respons yang valid 🥺"
      });
    } catch (error) {
      res.status(500).json({
        status: false,
